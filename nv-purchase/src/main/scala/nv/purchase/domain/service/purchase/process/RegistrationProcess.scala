...
import nv.purchase.domain.service.purchase.PurchaseProcessManager.Events.{ CancelRegistrationProcessed, OrderProcessed, RegistrationProcessed }
...
trait RegistrationProcess {
  ...
  when(RegistrationProcessing, 5.seconds) {
    ...
    case Event(BuyCompleted, _) ⇒
      goto(PurchaseCompleted) applying RegistrationProcessed() forMax 10.seconds

    /**
      * 登録処理のキャンセルが完了したら、オーダー処理(OrderProcessing)へ戻る。
      */
    case Event(CancelCompleted, _) ⇒
      goto(OrderProcessing) applying CancelRegistrationProcessed() forMax 1.second
