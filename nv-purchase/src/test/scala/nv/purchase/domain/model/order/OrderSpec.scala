...
import nv.common.ddd.domain.AggregateRoot.Commands.{ SetState, GetState }
...
class OrderSpec extends TestSupport {
  ...
  "Placed Order" should {
    "cancel" in {
      val id = Order.nextId
      val order = system.actorOf(Props[Order])
      order ! SetState(OrderState(Set(Item(ProductId("1"), 100, 10), Item(ProductId("2"), 300, 20)), placed = true))
